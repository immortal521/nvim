...
▀▄     ▀▄  █     ▀██▀     ██▄█
 ...
 █   █  █      ▄▄           ▄▀
				...
				keys = {
					...
					{ icon = " ", key = "p", desc = "Pack", action = ":ZShow" },
					{
						icon = " ",
						key = "S",
						desc = "Select Session",
	...
	keys = {
		...
		{
			"<leader>fB",
			"<cmd>lua Snacks.picker.buffers({ hidden = true, nofile = true })<cr>",
			desc = "Buffers (all)",
		},
		{
			"<leader>fc",
			"<cmd>lua Snacks.picker.pick('files', { cwd = vim.fn.stdpath('config') })<cr>",
